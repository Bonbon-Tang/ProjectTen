...
class EvaluationService:
    ...
    def start(db: Session, task: EvaluationTask) -> EvaluationTask:
        ...
        TaskRuntimeService.append_log(
            db,
            task_type="evaluation",
            task_id=task.id,
            task_run_id=run.id,
            stage="preparing",
            message=f"任务已启动，准备执行环境：{task.device_type or 'unknown'} x{task.device_count or 1}",
        )
        return task
    ...
    def _run_simulation(task_id: int) -> None:
        ...
        try:
            ...
            run = TaskRuntimeService.latest_run(db, task_type="evaluation", task_id=task.id)
            if not run:
                run = TaskRuntimeService.start_run(
                    db,
                    task_type="evaluation",
            ...
            TaskRuntimeService.append_log(db, task_type="evaluation", task_id=task.id, task_run_id=run.id, stage="preparing", message="环境检查完成，开始进入执行队列")
            TaskRuntimeService.update_run_stage(db, run, stage="running")
            TaskRuntimeService.append_log(db, task_type="evaluation", task_id=task.id, task_run_id=run.id, stage="running", message="评测任务开始执行，正在采集运行指标")
            ...
            while elapsed < total_duration:
                ...
                if new_progress > task.progress:
                    ...
                    TaskRuntimeService.append_log(
                        ...
                        task_run_id=run.id,
                        stage="running",
                        message=f"执行中：当前进度 {new_progress}%",
            ...
            task.result = {
                ...
                "conclusion": {
                    "summary": "评测任务完成，已生成指标与报告。",
                    "recommended_next_step": "查看报告并决定是否写入榜单/进入适配。",
                },
            }
            task.progress = 100
            ...
            EvaluationService._release_devices(db, task)
            db.commit()
            TaskRuntimeService.append_log(db, task_type="evaluation", task_id=task.id, task_run_id=run.id, stage="collecting", message="指标采集完成，正在生成报告与结果详情")
            ...
            try:
                ...
                if task:
                    ...
                    task.result = {
                        ...
                        "failure_summary": "评测执行失败",
                        "suggestions": ["检查镜像与设备是否匹配", "查看运行日志与参数配置"],
                    }
                    EvaluationService._release_devices(db, task)
                    db.commit()
                    run = TaskRuntimeService.latest_run(db, task_type="evaluation", task_id=task.id)
                    if run:
                        TaskRuntimeService.append_log(db, task_type="evaluation", task_id=task.id, task_run_id=run.id, stage="failed", level="ERROR", message=f"执行失败：{str(e)}")
