...
class AdaptationService:
    ...
    def create(db: Session, *, creator_id: int, tenant_id: Optional[int] = None, **kwargs) -> AdaptationTask:
        ...
        db.commit()
        db.refresh(task)
        run = TaskRuntimeService.start_run(
            db,
        ...
        TaskRuntimeService.append_log(db, task_type="adaptation", task_id=task.id, task_run_id=run.id, stage="queued", message="适配任务已创建，等待环境准备")
    ...
    def _run_task(task_id: int) -> None:
        ...
        try:
            ...
            run = TaskRuntimeService.latest_run(db, task_type="adaptation", task_id=task.id)
            time.sleep(5)
            db.refresh(task)
            task.result = {
                "stage": "adapting",
            ...
            db.commit()
            if run:
                TaskRuntimeService.update_run_stage(db, run, stage="preparing")
                TaskRuntimeService.append_log(db, task_type="adaptation", task_id=task.id, task_run_id=run.id, stage="preparing", message="环境准备完成，开始生成适配策略")
            ...
            while elapsed < total_duration:
                ...
                db.commit()
                if run:
                    TaskRuntimeService.update_run_stage(db, run, stage="running")
                    TaskRuntimeService.append_log(db, task_type="adaptation", task_id=task.id, task_run_id=run.id, stage="running", message=f"适配执行中：当前进度 {progress}%", details={"progress": progress})
            ...
            actions = [
                {
                    "action_type": "switch_runtime",
                    "title": "切换运行时配置",
                    "before_value": {"runtime": "default", "test_mode": task.test_mode},
                    "after_value": {"runtime": "optimized", "test_mode": task.test_mode},
                    "reason": "匹配当前设备的推荐执行后端",
                    "status": "completed",
                },
                {
                    "action_type": "change_precision",
                    "title": "调整计算精度",
                    "before_value": {"precision": "fp32"},
                    "after_value": {"precision": task.precision},
                    "reason": "平衡性能与精度，降低资源占用",
                    "status": "completed",
                },
                {
                    "action_type": "adjust_batch_size",
                    "title": "调整批大小",
                    "before_value": {"batch_size": 1},
                    "after_value": {"batch_size": 4 if task.device_count == 1 else 8},
                    "reason": "提升吞吐并适配当前设备并发能力",
                    "status": "completed",
                },
            ]
            TaskRuntimeService.create_adaptation_actions(db, task.id, actions)
            ...
            task.result = {
                ...
                "adaptation_detail": {
                    "actions": actions,
                    "before": {"precision": "fp32", "runtime": "default", "batch_size": 1},
                    "after": {"precision": task.precision, "runtime": "optimized", "batch_size": 4 if task.device_count == 1 else 8},
                },
                "evaluation_detail": metrics,
                "conclusion": {
                    "summary": "适配流程已完成，并生成适配动作记录。",
                    "recommended_next_step": "确认是否保存镜像并写入榜单。",
                },
            }
            db.commit()
            if run:
                TaskRuntimeService.append_log(db, task_type="adaptation", task_id=task.id, task_run_id=run.id, stage="collecting", message="适配完成，正在整理适配动作与评测细节")
                TaskRuntimeService.finish_run(db, run, status="completed", stage="completed", result_snapshot=task.result)
            ...
            if task:
                ...
                task.result = {
                    ...
                    "success": False,
                    "message": str(e),
                    "failure_summary": "适配任务执行失败",
                    "suggestions": ["检查镜像兼容性", "检查精度与运行参数设置"],
                }
                db.commit()
                run = TaskRuntimeService.latest_run(db, task_type="adaptation", task_id=task.id)
                if run:
                    TaskRuntimeService.append_log(db, task_type="adaptation", task_id=task.id, task_run_id=run.id, stage="failed", level="ERROR", message=f"适配失败：{str(e)}")
                    TaskRuntimeService.finish_run(db, run, status="failed", stage="failed", result_snapshot=task.result, error_message=str(e))
        finally:
            db.close()
